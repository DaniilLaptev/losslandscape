...
def get_params(model, random=False, device='cpu'):
    parameters = []
    for param in model.parameters():
        if random:
            parameters.append(torch.rand(param.shape).flatten().to(device))
        else:
            parameters.append(param.clone().detach().flatten().to(device))
    return parameters
        
def set_params(model, p, device='cpu'):
    for mpar, vpar in zip(model.parameters(), p):
        mpar.data = nn.Parameter(vpar.reshape(mpar.shape).to(device))
...
class LossLandscapePlotting():
    def __init__(
        ...
        self.eigvals, self.vecs = self.get_eigvecs() if eigvecs else (None, None)
        
        if not eigvecs and vecs is not None:
            self.vecs = vecs

    def get_eigvecs(self):
        '''
        Computes eigenvectors of Hessian for model at certain point, provided as 
        self.theta0, and returns top n(=2) eigenvalues (not used) and eigenvectors.
        '''

        print('Computing eigenvectors at providen data...')
        hessian_comp = hessian(self.model.to('cpu'), self.criterion, data=self.data, cuda=False)
    ...
    def L(self, x, loss, theta1):
        ...
        newvec = (self.theta0 + self.vecs[0] * x[0] + self.vecs[1] * x[1])
        par_diff = (theta1 - newvec).params
        
        new_parameters = (self.theta0 + self.vecs[0] * x[0] + self.vecs[1] * x[1]).params
    ...
    def compute_trace(self, every_ith=1, method='nelder-mead'):
        ...
        for theta1, loss in tqdm(zip(parameters, losses), total=total_iters):
            guess = opt.minimize(self.L, (0, 0), args=(loss, theta1.to(self.device)), method=method).x

            new_par = (self.theta0 + self.vecs[0] * guess[0] + self.vecs[1] * guess[1]).params
    ...
    def compute_landscape(self, trace, grid_density=50, coef=1, arange=None, brange=None, make_equal=True):
        ...
        for alpha, beta in tqdm(product(ralpha, rbeta), total=grid_density*grid_density):
            new_params = (self.theta0 + self.vecs[0] * alpha + self.vecs[1] * beta).params
